...
const CircularProgress = ({
  ...
  return (
    <div className={`relative flex items-center justify-center w-40 h-40 mx-auto my-6 rounded-full transition-all duration-500 ${isAlarming ? "animate-pulse shadow-[0_0_30px_rgba(239,68,68,0.3)] bg-red-500/5" : ""}`}>
      <svg viewBox="0 0 160 160" className="absolute w-full h-full transform -rotate-90 origin-center">
...
export function TimerTab({ onModalOpenChange }: { onModalOpenChange?: (open: boolean) => void }) {
    ...
    try {
      ...
      data.forEach((dev: any) => {
        if (dev.status === "active" && dev.pricing_method === "time" && dev.duration > 0) {
          ...
          // Pause logic calculations
          const sId = dev.current_session_id;
          const accum = parseInt(localStorage.getItem(`pause_accum_${sId}`) || "0");
          const pauseStart = parseInt(localStorage.getItem(`pause_start_${sId}`) || "0");
          
          let activeElapsed = Math.floor((Date.now() - startTimeMs) / 1000) - accum;
          if (pauseStart > 0) {
             activeElapsed -= Math.floor((Date.now() - pauseStart) / 1000);
          }
          
          const secondsLeft = Math.max(0, totalSeconds - activeElapsed);
          ...
          activeTimers.push({
            ...
            icon: gameMeta.icon,
            isPaused: pauseStart > 0,
            isAlarming: secondsLeft <= 0 && pauseStart === 0,
  ...
  const handleStartTimer = async () => {
    ...
    const durationMin = parseInt(inputTime) || 0;
    const durationSec = parseInt(inputSeconds) || 0;
    const totalMinutes = durationMin + durationSec / 60;
    ...
    const totalCost = Math.round(totalMinutes * priceRate);
  ...
  const handleExtend = async (session: ActiveSession, additionalMinutes: number) => {
    ...
    // Keep the cost exactly the same as requested
    const newCost = session.cost;
  ...
  const handlePauseToggle = (session: ActiveSession) => {
    if (session.isPaused) {
      // Unpause
      const pauseStart = parseInt(localStorage.getItem(`pause_start_${session.id}`) || "0");
      if (pauseStart > 0) {
        const pauseDuration = Math.floor((Date.now() - pauseStart) / 1000);
        const accum = parseInt(localStorage.getItem(`pause_accum_${session.id}`) || "0") + pauseDuration;
        localStorage.setItem(`pause_accum_${session.id}`, accum.toString());
        localStorage.removeItem(`pause_start_${session.id}`);
      }
    } else {
      // Pause
      localStorage.setItem(`pause_start_${session.id}`, Date.now().toString());
    }
    
    setSessions(prev => prev.map(s => s.id === session.id ? { ...s, isPaused: !s.isPaused } : s));
  };
  ...
  const handleCancelTimer = async (session: ActiveSession) => {
    if (!confirm(`Are you sure you want to completely cancel the session for ${session.console}? This will NOT be billed or saved.`)) return;
    
    try {
      const res = await fetch("http://localhost:3000/api/sessions/cancel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          sessionId: session.id,
  ...
  return (
    <div className="w-full h-full flex flex-col relative select-none overflow-y-auto">
                  ...
                  <AnimatePresence>
                    {activeDropdownId === session.id && (
                      ...
                      >
                        ...
                        {/* Reduce Time Row */}
                        <div className="flex justify-between px-2 mb-1 gap-1">
                          {[5, 10, 15].map(mins => (
                            <button
                               key={`minus-${mins}`}
                               onClick={() => {
                               ...
                               className="flex-1 py-1 text-xs text-center text-red-400 bg-red-500/10 hover:bg-red-500/20 rounded-[8px] transition-colors"
                            >-{mins}</button>
                          ))}
                        </div>
                        
                        {/* Extend Time Row */}
                        <div className="flex justify-between px-2 mb-2 gap-1">
                          {[5, 10, 15].map(mins => (
                            <button
                               key={`plus-${mins}`}
                               onClick={() => {
                               ...
                               className="flex-1 py-1 text-xs text-center text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-[8px] transition-colors"
                            >+{mins}</button>
                          ))}
                        </div>
                        ...
                        <div className="px-2 pb-2">
                           <button
                             onClick={() => {
                               handleExtend(session, 60);
                               setActiveDropdownId(null);
                             }}
                             className="w-full text-center py-1 text-xs text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-[8px] transition-colors"
                           >
                             +1 Hour
                           </button>
                        </div>
                        
                        <div className="px-2 border-t border-zinc-200 dark:border-white/5 pt-2">
                           <button
                             onClick={() => {
                               handleCancelTimer(session);
                               setActiveDropdownId(null);
                             }}
                             className="w-full text-left px-3 py-2 text-sm text-red-500 hover:text-white hover:bg-red-600 rounded-[10px] transition-colors flex items-center gap-2"
                           >
              ...
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-zinc-200 dark:border-white/5">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`w-10 h-10 rounded-[14px] flex items-center justify-center transition-all duration-300 ${session.isPaused ? "bg-amber-500/20 text-amber-500 border border-amber-500/50" : "bg-zinc-500/10 text-zinc-400 hover:bg-amber-500 hover:text-white"}`}
      ...
      {/* Spacer + Divider */}
      <div className="w-full h-[1px] bg-black/5 dark:bg-white/5 mt-16 mb-8 flex-shrink-0" />
        ...
        {isAddModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-[#0f0f11] border border-zinc-200 dark:border-white/5 rounded-[28px] p-6 shadow-none z-10"
            >
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-[11px] text-[#9A031E]">
                      {(parseInt(inputTime) || 0) + (parseInt(inputSeconds) || 0) / 60 > 0
                        ? `${Math.round(((parseInt(inputTime) || 0) + (parseInt(inputSeconds) || 0) / 60) * (GAME_DATA[selectedGame]?.defaultRate || 5))} DA`
                        : "0 DA"}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-3 mb-3">
                    <div className="flex-1 flex flex-col gap-1">
                      <span className="text-[10px] text-zinc-500 dark:text-white/30 uppercase tracking-wider font-semibold">Minutes</span>
                      <input
                        type="number"
                        value={inputTime}
                        ...
                        placeholder="e.g. 60"
                        min="0"
                        className="w-full bg-white dark:bg-black/40 border border-zinc-200 dark:border-white/10 rounded-[16px] px-4 py-3.5 text-zinc-900 dark:text-white text-lg focus:outline-none focus:border-[#9A031E] transition-colors"
                        autoFocus
                      />
                    </div>
                    <div className="flex-1 flex flex-col gap-1">
                      <span className="text-[10px] text-zinc-500 dark:text-white/30 uppercase tracking-wider font-semibold">Seconds</span>
                      <input
                        type="number"
                        value={inputSeconds}
                        onChange={(e) => {
                  ...
                  <div className="flex gap-2">
                    {[20, 40, 60, 120].map((mins) => (
                      <button
                        key={mins}
                        onClick={() => { setInputTime(String(mins)); setInputSeconds("0"); }}
                        className={`flex-1 py-2 text-xs font-semibold rounded-[12px] border transition-all duration-300 ${inputTime === String(mins) && inputSeconds === "0"
              ...
              <div className="flex gap-4 mt-8">
                ...
                <button
                  ...
                  disabled={!selectedDeviceId || ((!inputTime || parseInt(inputTime) === 0) && (!inputSeconds || parseInt(inputSeconds) === 0))}
