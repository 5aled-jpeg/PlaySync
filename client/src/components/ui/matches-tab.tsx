...
export function MatchesTab({ onModalOpenChange }: { onModalOpenChange?: (open: boolean) => void }) {
  ...
  // Synthesize Referee Whistle Sound when match ends
  const playWhistleSound = () => {
    ...
    try {
      ...
      const playWhistle = (time: number, duration: number) => {
        ...
        osc.type = "sine";
        osc.frequency.setValueAtTime(3000, time); // High pitched clean whistle
        osc.frequency.exponentialRampToValueAtTime(2800, time + duration);
      ...
      // Traditional double referee whistle
      playWhistle(now, 0.15);
      playWhistle(now + 0.2, 0.4);
    } catch (e) {
      console.error("Failed to play whistle sound:", e);
    }
  };
      ...
      <header className="flex justify-between items-center mb-8 flex-shrink-0">
        ...
        <div className="flex gap-4">
          <button
            ...
            className={`w-12 h-12 rounded-[16px] flex items-center justify-center border transition-all duration-300 ${
              isMuted 
            ...
            title={isMuted ? "Mute Whistle Alert" : "Unmute Whistle Alert"}
          >
        ...
        {isAddModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-[#0b0f0c] border border-emerald-500/10 rounded-[28px] p-6 shadow-[0_0_50px_rgba(16,185,129,0.15)] z-10"
            >
