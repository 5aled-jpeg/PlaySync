...
function App() {
          ...
          fetch("http://localhost:3000/api/history/all")
            ...
            .then(transactions => {
              ...
              const rows = transactions.map((t: any) => [
                ...
                t.duration ? `${t.duration} ${t.pricing_method === 'match' ? 'session' : 'Mins'}` : "N/A",
                t.amount
              ])
  ...
  return (
    <div className="relative w-full h-screen min-h-[600px] flex flex-col items-center bg-black text-[#FFFFFC] font-sans rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
      ...
      {/* Solid Top Bar (Blur removed for performance) */}
      <div className={`w-full h-24 flex items-center justify-between px-8 border-b border-white/5 bg-[#0a0a0a] transition-all duration-150 relative ${
        isAnyModalActive ? "opacity-0 scale-95 pointer-events-none" : "opacity-100 scale-100"
      }`}>
            ...
            {TAB_CONTENT.map((tab, index) => {
              ...
              return (
                ...
                >
                  <span
                    className={`
                    w-full h-full flex items-center justify-center gap-2 font-medium transition-colors duration-150
      ...
      <div className="w-full flex-1 p-8 overflow-hidden relative">
        <AnimatePresence mode="popLayout">
          <motion.div
            ...
            className="w-full h-full bg-[#111111] border border-white/5 rounded-[24px] p-8 flex flex-col"
          >
            {activeTab === 0 ? (
      ...
      <AnimatePresence>
        {alarmingSession && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            {/* Solid dark back drop for performance */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-[#0c0506] border border-red-500/20 rounded-[32px] p-8 shadow-none z-10 overflow-hidden flex flex-col"
            >
